...
3. Use these settings:
   - **Build Command**: `npm cache clean --force && rm -rf node_modules && mv prisma.config.ts prisma.config.ts.bak 2>/dev/null || true && npm ci && rm -rf node_modules/.prisma && PRISMA_CLIENT_ENGINE_TYPE=library npx prisma generate && mv prisma.config.ts.bak prisma.config.ts 2>/dev/null || true && npx prisma db push && npm run build`
